...
def register():
    ...
    if request.method == "POST":
        ...
        result = db.execute("INSERT INTO USERS (username, password) VALUES (:username :password)", username=request.form.get("username"), password=password)
        if not result:
            return apology("That username has already been taken. Sorry!",400)
